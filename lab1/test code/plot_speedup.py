...
df = pd.read_csv('benchmark_results.csv', names=['workload', 'impl', 'threads', 'size', 'arraysize', 'run_i', 'elapsed_ms', 'notes'], skiprows=1)

avg_times = df.groupby(['impl', 'threads'])['elapsed_ms'].mean().reset_index()

serial_time = avg_times[avg_times['impl'] == 'serial']['elapsed_ms'].mean()

openmp_data = avg_times[(avg_times['impl'] == 'openmp') & (avg_times['threads'] > 0)].copy()
threads_data = avg_times[(avg_times['impl'] == 'threads') & (avg_times['threads'] > 0)].copy()
...
plt.title('Хурдны өсөлтийн харуулсан график', fontsize=14, fontweight='bold')
...
print("График амжилттай үүсгэгдлээ: speedup_graph.png")
